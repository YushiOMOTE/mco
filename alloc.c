...
int 
liballoc_free (void *p, int pages)
{
	return page_free (p, pages);
}
...
static inline struct boundary_tag* split_tag( struct boundary_tag* tag )
{
	...
	struct boundary_tag *new_tag = 
		(struct boundary_tag*)((void *)tag + sizeof(struct boundary_tag) + tag->size);	
...
void *malloc(size_t size)
{
	...
	ptr = (void*)((void *)tag + sizeof( struct boundary_tag ) );
...
void free(void *ptr)
{
	...
	liballoc_lock();
		...
		tag = (struct boundary_tag*)((void *)ptr - sizeof( struct boundary_tag ));
...
void*   realloc(void *p, size_t size)
{
	...
	liballoc_lock();
		tag = (struct boundary_tag*)((void *)p - sizeof( struct boundary_tag ));
		real_size = tag->size;
	liballoc_unlock();
